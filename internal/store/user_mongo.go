...
func (ur *UserRepoMongo) GetUserByID(ctx context.Context, id string) (*models.User, error) {
	...
	err = ur.col.FindOne(ctx, bson.M{"id": objID}).Decode(&user)
	if err != nil {
		return nil, err
	}
...
func (ur *UserRepoMongo) UpdateUser(ctx context.Context, id string, user models.User) (*models.User, error) {
	...
	result, err := ur.col.UpdateOne(ctx, bson.M{"id": objID}, bson.M{"$set": update})
	...
	if result.MatchedCount == 1 {
		err := ur.col.FindOne(ctx, bson.M{"id": objID}).Decode(&updatedUser)
...
func (ur *UserRepoMongo) DeleteUser(ctx context.Context, id string) error {
	...
	result, err := ur.col.DeleteOne(ctx, bson.M{"id": objID})
	if err != nil {
		return fmt.Errorf("error deleting using: %w", err)
	}

	if result.DeletedCount < 1 {
		return fmt.Errorf("user with specified ID not found: %w", err)
	}
